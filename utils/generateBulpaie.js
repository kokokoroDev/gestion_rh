...
const drawHeader = (doc, bulpaie) => {
    ...
    const moduleName = salarie?.module?.libelle ?? 'N/A';
    ...
    // Period badge
    const period = periodLabel(bulpaie.month, bulpaie.year);
...
const drawEmployeeSection = (doc, bulpaie) => {
    ...
    return infoY + lineH + 12; // return next y
};

const drawPayTable = (doc, bulpaie, startY) => {
    const tableX = 50, tableW = 495;
    const col1W = 300, col2W = 195; // label | amount
    ...
    rows.forEach((row, i) => {
        ...
        // Subtle left border accent for gain rows
        if (row.type === 'gain') {
            fillRect(doc, tableX, rowY, 3, 22, COLORS.accent);
        }
    ...
    doc.font('Helvetica-Bold').fontSize(13).fillColor('#93C5FD') // light blue for contrast
        .text(`${fmt(bulpaie.salaire_net)} MAD`, tableX + col1W + 10, netY + 9, {
            width: col2W - 20, align: 'right'
        });

    return netY + 32 + 20; // return next y
};

const drawValidationStamp = (doc, nextY) => {
    // Dashed border box
    doc.save()
        .rect(350, nextY, 195, 52)
        .lineWidth(1.2)
        .dash(4, { space: 3 })
...
export const generatePayslipPDF = (bulpaie) => {
