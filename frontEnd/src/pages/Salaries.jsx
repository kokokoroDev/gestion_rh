...
export default function Salaries() {
  ...
  const load = useCallback(() => {
    ...
    // Manager is scoped to his own team on the backend via GET /salarie/team
    // RH gets everyone via GET /salarie
    if (isManager && !isRH) {
  ...
  // Fonctionnaire has no access to the employee list
  if (isFonctionnaire) {
    return (
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="card flex flex-wrap gap-3">
        ...
        <button
          onClick={() => { setFilters({ role: '', status: 'active' }); setSearch(''); setPage(0) }}
          className="btn-ghost text-sm"
        >
          Réinitialiser
        </button>
        <div className="ml-auto text-sm text-surface-400">{total} salarié{total !== 1 ? 's' : ''}</div>
      </div>
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                    ...
                    {s.module?.libelle && (
                      <Badge className="bg-surface-100 text-surface-500">{s.module.libelle}</Badge>
                    )}
                    {s.status === 'inactive' && (
              ...
              {isRH && (
                <div className="mt-3 flex gap-2" onClick={(e) => e.stopPropagation()}>
                  ...
                  {confirmDel === s.id ? (
                    <>
                      <button onClick={() => handleDelete(s.id)} disabled={deleting} className="btn-danger text-xs flex-1 py-1.5">
                        {deleting ? <Spinner size="sm" /> : 'Confirmer'}
                      </button>
                      <button onClick={() => setConfirmDel(null)} className="btn-secondary text-xs py-1.5 px-3">Non</button>
                    </>
                  ) : (
                    <button
