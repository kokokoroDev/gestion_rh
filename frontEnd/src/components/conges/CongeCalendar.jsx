...
export default function CongeCalendar() {
    ...
    // moduleId is now derived from roleModules inside useAuth
    ...
    useEffect(() => {
        const params = {
            ...
            date_to:   range.to,
            // Non-RH users: scope calendar to their first module (if any)
        ...
        <div className="space-y-4">
            ...
            <div className="card flex flex-wrap items-end gap-4">
                <div>
                    ...
                    <input type="date" className="input-base w-44" value={range.from}
                        onChange={(e) => setRange((r) => ({ ...r, from: e.target.value }))} />
                </div>
                <div>
                    <label className="block text-xs font-medium text-surface-500 mb-1">Au</label>
                    <input type="date" className="input-base w-44" value={range.to} min={range.from}
                        onChange={(e) => setRange((r) => ({ ...r, to: e.target.value }))} />
                </div>
                {loading && <Spinner size="sm" className="text-azure-500 mb-2" />}
                {!isRH && moduleId && (
                    <span className="text-xs text-surface-400 mb-2">Affichage limité à votre département</span>
                )}
                {!isRH && !moduleId && (
                                ...
                                <tbody className="divide-y divide-surface-50">
                                    {mod.absences.map((a, i) => (
                                        <tr key={i} className="hover:bg-surface-50 transition-colors">
                                            <td className="py-2.5 pr-4 font-medium text-surface-800">
                                                {a.sal_id === salarie?.id ? 'Moi' : a.nom}
                                            </td>
